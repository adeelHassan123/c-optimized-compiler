...
static IRValue generate_ir_expr(IRCode *code, ASTNode *expr) {
    ...
    switch (expr->type) {
        ...
        case NODE_CALL: {
            /* Module 3: Function calls (log and exp) */
            IRValue arg = generate_ir_expr(code, expr->data.unop.operand);
            IRValue result = ir_alloc_temp();
            
            /* Emit function call as a special binop with function name as op code */
            ir_emit(code, IR_CALL, result, arg, arg, expr->data.unop.op);
            return result;
        }
...
static const char *tok_op_to_str(int op_type) {
    switch (op_type) {
        ...
        case TOK_DIV: return "/";
        case TOK_MOD: return "%";
        case TOK_EXP: return "^";  /* Module 3: Exponentiation */
        case TOK_EQ: return "==";
        case TOK_NEQ: return "!=";
        case TOK_LT: return "<";
        case TOK_LE: return "<=";
        ...
        case TOK_OR: return "||";
        case TOK_NOT: return "!";
        case TOK_LOG: return "log";  /* Module 3 */
        case TOK_EXP_FUNC: return "exp";  /* Module 3 */
        default: return "?";
    }
}
